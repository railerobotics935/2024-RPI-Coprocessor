...
#                print(f"{depthFrame.shape[1]} : {depthFrame.shape[0]}") # should be 640 x 400
                ...
                if frame is not None:
                    ...
                    # Ignore any tags not in the set used on the 2023 FRC field:
                    apriltags = [tag for tag in apriltags if ((tag.getId() > 0) & (tag.getId() < 9))]
...
#                            print(f"{tag_id}: {pose}")
                            ...
                            if ((tag_id > 0) & (tag_id < 9)):
                                ...
                                ssd.putNumberArray("Location", [int(center.x - nn_width / 2), int(center.y - nn_height / 2), int(tag_distance)])
                        ...
                        for i in range(len(tag_state)):
                            if tag_state[i] == "LOST" :
                                ...
                                ssd.putNumberArray("Location", [0, 0, 0])
