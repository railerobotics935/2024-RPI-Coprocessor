...
#    cs.enableLogging()
    ...
    # OAK-D-S2 use ISP scaler 1/4: 4032x3040 -> 1008x760
    isp_denom = 1
    isp_divid = 4
    color_width = 1008
    color_height = 760
    ...
    if wideFOV:
        ...
        camRgb.setIspScale(isp_denom, isp_divid)
        camRgb.setPreviewSize(color_width, color_height)
...
#                print(f"{depthFrame.shape[1]} : {depthFrame.shape[0]}") # should be 640 x 400
                ...
                #            sd.putNumber("Confidence", int(detection.confidence * 100))
                        ...
                        for tag in apriltags:
                            ...
                            # rgbFrame = 416 x 416
                            tag_to_depth_scale = mono_width / float(color_width)
                            tag_in_depth_x = int(center.x * tag_to_depth_scale)
                            tag_in_depth_y = int(center.y * tag_to_depth_scale - (((color_height/2)*tag_to_depth_scale)-(mono_height/2)))
                            if tag_in_depth_x < 0 :
                                tag_in_depth_x = 0
                            if tag_in_depth_x > (mono_width - 1) :
                                tag_in_depth_x = (mono_width - 1)
                            if tag_in_depth_y < 0 :
                                tag_in_depth_y = 0
                            if tag_in_depth_y > (mono_height - 1) :
                                tag_in_depth_y = (mono_height - 1)
